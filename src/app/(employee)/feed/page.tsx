...
const FEATURED_PERKS = [
  { brand: 'GE Appliances', tagline: 'Save an additional 15%', bgColor: '#f0f4ff', logo: 0 },
  { brand: 'Vitamix', tagline: 'Get 15% off orders of $300+', bgColor: '#fff8f8', logo: 1 },
  { brand: 'HexClad', tagline: 'Premium cookware — 20% employee discount', bgColor: '#f9fafb', logo: 2 },
  { brand: 'GE Appliances', tagline: 'Free delivery on orders over $499', bgColor: '#f0f4ff', logo: 0 },
  { brand: 'Vitamix', tagline: 'Extended warranty included', bgColor: '#fff8f8', logo: 1 },
  { brand: 'HexClad', tagline: 'Lifetime guarantee on all products', bgColor: '#f9fafb', logo: 2 },
  { brand: 'GE Appliances', tagline: 'Exclusive bundle deals this month', bgColor: '#f0f4ff', logo: 0 },
  { brand: 'Vitamix', tagline: 'Free shipping on all blenders', bgColor: '#fff8f8', logo: 1 },
];
...
const EXP_SLIDES = [
  {
    bgGradient: 'linear-gradient(120deg, #0c2d4a 0%, #0d4a6e 55%, #1565a8 100%)',
    panelBg: '#0f3460',
  ...
  {
    bgGradient: 'linear-gradient(120deg, #0f2d1a 0%, #1a4d2a 55%, #1e6b3a 100%)',
    panelBg: '#14532d',
      ...
      <div className={styles.heroSection}>
        <div className={styles.heroSlide} style={{ background: slide.bg }}>
          <div className={styles.heroContent}>
            ...
            <button className={styles.heroCta} style={{ color: slide.ctaColor }}>
              {slide.cta} &rarr;
            </button>
          </div>
          <div className={styles.heroBrandBox}>
            <span className={styles.heroBrandName} style={{ whiteSpace: 'pre-line' }}>
              {slide.brand}
            </span>
          </div>
        </div>
        ...
        {FEATURED_PERKS.map((perk, i) => {
          ...
          return (
            <a key={`${perk.brand}-${i}`} href="#" className={styles.perkCard}>
              <div className={styles.perkLogoArea} style={{ background: perk.bgColor }}>
                <LogoComponent />
              </div>
        ...
        <div className={styles.trendGrid}>
          {TRENDING_PERKS.map((perk) => (
            <a key={perk.brand} href="#" className={styles.trendCard}>
              <div className={styles.trendImgArea} style={{ background: perk.imgBg }}>
                {/* bokeh decoration for Cineplex */}
                {perk.bokeh && (
                  <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.35 }} viewBox="0 0 400 252" preserveAspectRatio="xMidYMid slice">
                    <circle cx="80" cy="60" r="45" fill="rgba(100,140,255,0.5)" />
                    <circle cx="200" cy="110" r="60" fill="rgba(80,120,220,0.4)" />
                    <circle cx="320" cy="50" r="38" fill="rgba(120,160,255,0.45)" />
                    <circle cx="140" cy="180" r="50" fill="rgba(60,100,200,0.35)" />
                    <circle cx="350" cy="190" r="42" fill="rgba(100,140,240,0.4)" />
                    <circle cx="50" cy="200" r="30" fill="rgba(140,180,255,0.3)" />
                  </svg>
                )}
      ...
      <div className={styles.expSection}>
        ...
        <div className={styles.expSlide}>
          ...
          <div className={styles.expBg} style={{ background: exp.bgGradient }}>
            <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.18 }} viewBox="0 0 700 290" preserveAspectRatio="xMidYMid slice">
              {/* simplified skyline silhouette */}
              <rect x="0" y="160" width="700" height="130" fill="rgba(0,0,0,0.3)" />
              <rect x="60" y="100" width="40" height="60" fill="rgba(255,255,255,0.08)" />
              <rect x="120" y="80" width="30" height="80" fill="rgba(255,255,255,0.06)" />
              <rect x="200" y="60" width="60" height="100" fill="rgba(255,255,255,0.07)" />
              <rect x="300" y="90" width="25" height="70" fill="rgba(255,255,255,0.05)" />
              <circle cx="350" cy="88" r="10" fill="rgba(255,255,255,0.06)" />
              <rect x="420" y="70" width="20" height="90" fill="rgba(255,255,255,0.07)" />
              <rect x="500" y="110" width="50" height="50" fill="rgba(255,255,255,0.05)" />
              <rect x="600" y="85" width="35" height="75" fill="rgba(255,255,255,0.06)" />
            </svg>
          </div>
          {/* info panel */}
          <div className={styles.expPanel} style={{ background: exp.panelBg }}>
            <p className={styles.expExclusive}>{exp.badge}</p>
            <h3 className={styles.expHeadline} style={{ whiteSpace: 'pre-line' }}>{exp.headline}</h3>
            <button className={styles.expCtaBtn} style={{ color: exp.ctaColor }}>{exp.cta}</button>
